...
| layout | 占位 | 适合 |
|---|---|---|
| `hero` | 全宽 + 白边「相纸」+ 居中大字题图 | **一段行程的定性之作，一段只放一张，放开头** |
...
```
hero → aside → center → duo → duo → full → left → aside → right → full
```

**`hero` 用法**：整段只放一张（一般是封面那张／最有分量的一张），摆在导语之后的第一位。
它的图注是居中大字，所以 `caption` 写短、`note` 写长。

**整段排序**：`data.json` 的 `trips` 数组要按**时间从早到晚**排（最早的在最前）。
首页年份分组也按从早到晚显示 —— 这是一本家庭史，从第一次出门开始往后读。
行程页的「上一段／下一段」直接依赖这个顺序，别乱放。
